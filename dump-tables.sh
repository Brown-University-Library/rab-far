...
do
  ...
  mysql -h ${DBLOC} -u ${USER} --password="${PASSW}" ${DBNAME} -e "select GROUP_CONCAT(COLUMN_NAME SEPARATOR '\`,\`') from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME='${table}'" > data/field-data.txt;
...
do
  echo "Selecting ${fields} from ${table}" && \
